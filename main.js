...
var RT_DEFAULT_SETTINGS = {
  ...
  format: "compact",
  appendText: "left",
  showProgressPercentage: true, // New toggle for reading progress percentage
};
var ReadingTimeSettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  .addToggle((toggle) => 
    toggle.setValue(this.plugin.settings.showProgressPercentage)
      .onChange(async (value) => {
        this.plugin.settings.showProgressPercentage = value;
        await this.plugin.saveSettings().then(this.plugin.calculateReadingTime);
      })
  );
...
function readingTimeText(text, plugin) {
  ...
  switch (plugin.settings.format) {
    ...
    case "digital":
      ...
      break;
    case "default":
      return plugin.settings.appendText ? `${result.minutes} min left` : `${result.minutes} min`;
  }
...
var ReadingTime = class extends import_obsidian2.Plugin {
  ...
  if (mdView && mdView.editor) {
    ...
    // Calculate text above the cursor
    ...
    // Extract remaining text and calculate reading time
    let textBelowCursor = "";
  ...
  onload() {
    return __async(this, null, function* () {
      ...
      // Recalculate reading time when the cursor moves or layout changes
      this.registerEvent(
      ...
      // Track cursor position and periodically check every 500 ms
      ...
      setInterval(() => {
        ...
        if (mdView && mdView.editor) {
          ...
          // If the cursor position has changed, recalculate the reading time
          if (lastCursorPosition === null || cursor.line !== lastCursorPosition.line || cursor.ch !== lastCursorPosition.ch) {
